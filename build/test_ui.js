...
/* 程式檔不含客戶資料，測試前先注入資料檔（等同使用者首次匯入）。 */
try { w.localStorage.setItem('dsipharm:dataset', fs.readFileSync('pharmacy-data.json', 'utf8')); }
catch (e) { console.log('⚠ 無法注入資料檔：', e.message); process.exit(1); }
...
function done() {
  click(btn('拜訪前'));
  setTimeout(() => {
    /* 取優先序第一家開卡，不寫死客戶名（客戶名屬客戶資料，不進 repo）。 */
    click([...d.querySelectorAll('button')].filter(b => /查看客戶卡|客戶卡|進入/.test(b.textContent))[0]
          || [...d.querySelectorAll('[role=button],button')].find(b => b.textContent.trim().length > 2 && /體系|藥局|生技|藥品/.test(b.textContent)));
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        click(btn('拜訪前'));
        setTimeout(() => {
          /* 取非優先序的一般客戶開卡（自動議題路徑），同樣不寫死客戶名。 */
            click([...d.querySelectorAll('button')].filter(b => /體系|藥局|生技|藥品/.test(b.textContent.trim())).slice(-1)[0]);
          setTimeout(() => {
...
function step() {
  ...
  setTimeout(() => {
    ...
    /* 只看字數會漏掉「畫面有字但資料沒接上」。v2.0.0 曾因衍生值在模組層求值
       （當時資料還沒匯入）而使客戶選單全空，字數檢查照樣通過。這裡實際斷言。 */
    if (t.includes('接單')) {
      const sel = [...d.querySelectorAll('select')].find((s) => s.options.length > 3);
      const n = sel ? sel.options.length : 0;
      console.log('  ↳ 客戶選單'.padEnd(16), String(n).padStart(6), n >= 22 ? 'OK' : (fail++, '⚠ 資料檔沒接上'));
    }
    step();
  }, 350);
}
