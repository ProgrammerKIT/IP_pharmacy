...
# ─────────────────────────────────────────────────────────────
CUTOFF_PERIOD = 8          # 官方 YoY 比較期間：兩年皆 1–8 月（裁定 1、23；2026/09/03 Kit 裁定升級）
...
ROSTER_FILE = os.environ.get('ROSTER_FILE', 'roster_src.json')
if not os.path.exists(ROSTER_FILE):
    raise SystemExit(
        f'找不到客戶名單檔 {ROSTER_FILE}。\n'
        '客戶名單屬客戶資料，不隨程式碼進入 repo，須由使用者自行保管並放在執行目錄。')
_roster = json.load(open(ROSTER_FILE, encoding='utf-8'))
EXCLUDE_CUSTOMERS = _roster['excludeCustomers']   # 排除客戶（裁定 8）
GROUP_MAP = _roster['groupMap']                   # 客戶合併鎖定表（34 原始名稱 → 22 客戶群）
