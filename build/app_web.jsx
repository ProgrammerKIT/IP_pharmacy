...
const CHANGELOG = [
  ...
  ['2.10.2', '2026-09-25', '建置流程加入洩漏掃描硬閘門：程式檔含任何客戶名稱、地址或單價即中止建置，產不出可部署檔案。關鍵字取自客戶名單檔而非手打清單，新增客戶自動納入掃描'],
  ['2.10.1', '2026-09-25', '分頁順序調整：排程移到複盤左邊，改為 接單／拜訪前／排程／複盤／資料——排程是每週用的，複盤是月底用的，依使用頻率排'],
  ['2.10.0', '2026-09-25', '每條線加上「補貨點」：間隔法日期，以及批量修正後的消化法日期（兩者差不到 7 天只顯示一個）。往後移＝這批拿得多先盯去化、往前移＝訂得少要提早去。說明欄同步載明兩法定義與回測誤差（中位 8–10 天，是估計不是承諾）'],
  ...
  ['2.5.0', '2026-09-20', '客戶卡議題顯示進度：若該議題在拜訪紀錄中出現過同名者，帶出最近一次的日期、結果與備註。已談出結果的標綠、仍「沒談到」的標灰並提示這次要談。原本劇本與紀錄各自獨立，已談完的議題會被重問、已推進的當成沒發生'],
...
function LineRow({ x, warn, dim }) {
  ...
  return (
    <div style={{ background: warn ? C.amberBg : (dim ? 'transparent' : '#F7FAFB'),
      ...
      {(() => {
        /* 補貨點：間隔法＝末單＋訂單平均相隔−7；消化法再乘上「本批÷平均每批」。
           兩法差距小於 7 天時只給一個日期，避免每列都掛兩個看似不同其實一樣的數字。
           回測 263 筆：消化法中位誤差 8.4 天、間隔法 9.8 天——兩者都只是估計，
           日期寫出來是為了抓大概的時點，不是承諾那一天會見底。 */
        if (!x.d_int || !x.d_dep) return null;
        const diff = Math.round((new Date(x.d_dep) - new Date(x.d_int)) / 86400000);
        const same = Math.abs(diff) < 7;
        return (
          <div style={{ fontFamily: MONO, fontSize: 10.5, color: dim ? C.ink3 : C.ink2, marginTop: 2 }}>
            補貨點 <b style={{ color: dim ? C.ink3 : C.ink }}>{String(x.d_int).slice(5)}</b>
            {same ? '（常態）' : (
              <>
                <span style={{ color: C.ink3 }}>（常態）→ </span>
                <b style={{ color: diff > 0 ? C.amber : C.teal }}>{String(x.d_dep).slice(5)}</b>
                <span style={{ color: C.ink3 }}>
                  （{diff > 0 ? `這批拿得多，晚 ${diff} 天` : `這批拿得少，早 ${-diff} 天`}）
                </span>
              </>
            )}
          </div>
    ...
    <div className="p-4">
      ...
      <div style={{ marginTop: 18, background: C.surf, border: `1px solid ${C.rule}`, padding: '13px 15px' }}>
        ...
        <div style={{ fontSize: 12.5, color: C.ink2, lineHeight: 1.95, marginTop: 6 }}>
          ...
          <b style={{ color: C.ink }}>補貨點 07-29（常態）→ 08-29</b>　估計該回頭補貨的日子。
          前者只看訂貨節奏（末單 ＋ 訂單平均相隔 − 7）；後者再乘上「本批 ÷ 平均每批」，
          反映這次拿多還是拿少。兩者差不到 7 天就只顯示一個。<br />
          <span style={{ color: C.ink3 }}>
            <b style={{ color: C.amber }}>往後移</b>＝這批拿得多，別急著去，先盯去化；
            <b style={{ color: C.teal }}>往前移</b>＝這次訂得少，可能在試水溫或分單，提早去看發生什麼事。
            <b style={{ color: C.ink2 }}>兩個都是估計值</b>——263 筆回測的中位誤差約 8 到 10 天，
            拿來抓大概的時點，不是承諾那天會見底。
          </span><br />
          <span style={{ color: C.ink3 }}>
...
/* ── 外殼 ─────────────────────────────────────────────── */
const NAV = [['entry', '🔥接單'], ['prep', '拜訪前'], ['sched', '排程'], ['review', '複盤'], ['data', '資料']];
